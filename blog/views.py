...
def post_list(request, tag_slug=None):
    object_list = Post.published.all() # take all posts
    tag = None

    if tag_slug:
        tag = get_object_or_404(Tag, slug=tag_slug) # we get the tag object with the given slug using the get_obj_or_404 shortcut
        object_list = object_list.filter(tags__in=[tag]) # for filtering the list of posts by ones
